...
/* print selected stats info into log files. */
static void filteringLog(const char* file, PDBGGUISTATSNODE pNode)
{
    static int lines = 0;
    if ( !lines )
    {
        dbg_log(file, "%8s%16s%16s%16s%16s%16s%16s%16s%16s%16s%16s%16s%16s%16s%16s\n", 
			 "Time", "HwAcc_exec", "delta", "RAW_exec",  "delta",  "REM_exec",   "delta",  "cNsExecuting", "delta", "cNsHalted", "delta", "cNsOther", "delta", "cNsTotal", "delta");
    }
    lines++;
    //dbg_log("00_dbg1.log", "%s\n", pNode->pszName);
    /* Time  RAW_exec  delta  HwAcc_exec   delta  REM_exec   delta  cNsExecuting   delta  cNsHalted   delta  cNsOther   delta  cNsTotal   delta */

    const char* format = "%16lld%16lld";
    if ( strcmp(pNode->pszName, "PageBitmap")==0 )
    {
        /* the first node, dump the logging time */
        time_t timep;
        struct tm *p;
        time(&timep);
        p = localtime(&timep);
        dbg_log(file, "%02d:%02d:%02d", p->tm_hour, p->tm_min, p->tm_sec);
    }
    else if ( strcmp(pNode->pszName, "cInsHwAcc")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cInsRAW")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cInsREMa")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cNsExecuting")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cNsHalted")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cNsOther")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cNsTotal")==0 )
    {
        dbg_log(file, format, pNode->Data.Counter.c, pNode->i64Delta);
    }
    else if ( strcmp(pNode->pszName, "cUrbsInPool")==0 )
    {
        /* the last item in the stats console. log a CR to start a new line. */ 
        dbg_log(file, "\n");
    }
    else {;}
                                  ...
                                  STAMVISIBILITY enmVisibility, const char *pszDesc, void *pvUser)
{
    ...
    /* log the selected stat items into log files. */
    filteringLog("00_dbg.log", pNode);
