...
int pdmR3DrvInit(PVM pVM)
{
    ...
    if (fLoadBuiltin)
    {
        ...
        dbg_log("00_driver.log", " ####  %s ---> %s\n", "loadbuildin", pszFilename);
    ...
    for (PCFGMNODE pCur = CFGMR3GetFirstChild(pDriversNode); pCur; pCur = CFGMR3GetNextChild(pCur))
    {
         ...
         */
        dbg_log("00_driver.log", " ####  Load additional driver modules. %s ---> %s\n", szFilename, szName);
        RegCB.pCfgNode = pCur;
    ...
    dbg_log("00_driver.log", " ####  %s ---> %s\n", "pdmR3DrvInit: returns VINF_SUCCESS", "OK");
...
static int pdmR3DrvLoad(PVM pVM, PPDMDRVREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    dbg_log("00_driver.log", " ####  pdmR3DrvLoad : %s ---> %s\n", pszFilename, pszName);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            dbg_out("00_driver.log", " #### PDM: Calling VBoxDriversRegister %s ---> %s\n", pszFilename, pszName);
...
static DECLCALLBACK(int) pdmR3DrvRegister(PCPDMDRVREGCB pCallbacks, PCPDMDRVREG pReg)
{
    ...
    if (pDrv)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            dbg_log("00_driver.log", "PDM: Registered driver '%s'\n", pReg->szName);
                ...
                if (pNew)
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        *ppBaseInterface = &pNew->IBase;
                        if (pLun)
                        {
                            dbg_log("00_driver.log", "PDM: Attached driver %p:'%s'/%d to LUN#%d on device '%s'/%d, pDrvAbove=%p:'%s'/%d\n",
                        ...
                        {
                            dbg_log("00_driver.log", "PDM: Attached driver %p:'%s'/%d, pDrvAbove=%p:'%s'/%d\n",
                                 pNew, pDrv->pReg->szName, pNew->iInstance,
                                 pDrvAbove, pDrvAbove ? pDrvAbove->pReg->szName : "", pDrvAbove ? pDrvAbove->iInstance : UINT32_MAX);
