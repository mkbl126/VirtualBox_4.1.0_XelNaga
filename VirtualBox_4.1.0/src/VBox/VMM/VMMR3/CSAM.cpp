...
VMMR3DECL(int) CSAMR3Init(PVM pVM)
{
    ...
    /* new stats items added here, by hq*/
    STAM_REG(pVM, &pVM->csam.s.cDangerousInstr,       STAMTYPE_U32, "/CSAM/DangerousInstrArray/_DangerousInstrCount",       STAMUNIT_COUNT, "The number of the dangerous instructions.");
    STAM_REG(pVM, &pVM->csam.s.iDangerousInstr,       STAMTYPE_U32, "/CSAM/DangerousInstrArray/_DangerousInstrIndex",       STAMUNIT_COUNT, "The index of the current the dangerous instructions.");
    char instr_path[32] = "/CSAM/DangerousInstrArray/";
    for ( int i=0; i<CSAM_MAX_DANGR_INSTR; i++ )
    {
        char item = (char)('a'+i);
        instr_path[26] = item;
        instr_path[27] = '\0';
        STAM_REG(pVM, &pVM->csam.s.aDangerousInstr[i],       STAMTYPE_U32, instr_path,       STAMUNIT_NONE, "The element of the dangerous instructions array.");
    }
