...
static STAMCOUNTER    gStatReSched;
static STAMCOUNTER    gStatSchedRAW;
static STAMCOUNTER    gStatSchedREM;
static STAMCOUNTER    gStatSchedHwAcc;

STAMCOUNTER    gStatcInsTotal;
STAMCOUNTER    gStatcInsREM;
STAMCOUNTER    gStatcInsRAW;
STAMCOUNTER    gStatcInsHwAcc;
...
# define EM_REG_PROFILE_ADV(a, b, c) \
        ...
        EM_REG_COUNTER(&pStats->StatRZInterpretSucceeded,    "/EM/CPU%d/RZ/Interpret/Success",           "The number of times an instruction was successfully interpreted.");
        EM_REG_COUNTER(&pStats->StatR3InterpretSucceeded,    "/EM/CPU%d/R3/Interpret/Success",           "The number of times an instruction was successfully interpreted.");
        ...
        EM_REG_PROFILE(&pVCpu->em.s.StatIOEmu,                 "/PROF/CPU%d/EM/Emulation/IO",      "Profiling of emR3RawExecuteIOInstruction.");
        EM_REG_PROFILE(&pVCpu->em.s.StatPrivEmu,               "/PROF/CPU%d/EM/Emulation/Priv",    "Profiling of emR3RawPrivileged.");
        ...
        /* the stats for reschedule process. */
        EM_REG_COUNTER(&gStatReSched,                                "/PROF/CPU%d/EM/EIF/ReSched",      "Number of the times for Guest Code ReSchedule.");
        EM_REG_COUNTER(&gStatSchedRAW,                             "/PROF/CPU%d/EM/EIF/SchedRAW",      "Number of the times for RAW exec.");
        EM_REG_COUNTER(&gStatSchedREM,                              "/PROF/CPU%d/EM/EIF/SchedREM",      "Number of the times for REM exec.");
        EM_REG_COUNTER(&gStatSchedHwAcc,                             "/PROF/CPU%d/EM/EIF/SchedHwAcc",      "Number of the times for HwAcc exec.");

        gStatcInsTotal.c = gStatcInsRAW.c = gStatcInsRAW.c = gStatcInsHwAcc.c = 0;
        EM_REG_COUNTER(&gStatcInsTotal,                              "/PROF/CPU%d/EM/EIF/cInsTotal",      "Number of the instructions for executed in GC.");
        EM_REG_COUNTER(&gStatcInsRAW,                                "/PROF/CPU%d/EM/EIF/cInsRAW",      "Number of the instructions for RAW exec.");
        EM_REG_COUNTER(&gStatcInsREM,                             "/PROF/CPU%d/EM/EIF/cInsREM",      "Number of the instructions for REM exec.");
        EM_REG_COUNTER(&gStatcInsHwAcc,                             "/PROF/CPU%d/EM/EIF/cInsHwAcc",      "Number of the instructions for HwAcc exec.");

#endif /* VBOX_WITH_STATISTICS */

        EM_REG_PROFILE(&pVCpu->em.s.StatForcedActions,     "/PROF/CPU%d/EM/ForcedActions",     "Profiling forced action execution.");
        EM_REG_PROFILE(&pVCpu->em.s.StatHalted,            "/PROF/CPU%d/EM/Halted",            "Profiling halted state (VMR3WaitHalted).");
        EM_REG_PROFILE_ADV(&pVCpu->em.s.StatCapped,        "/PROF/CPU%d/EM/Capped",            "Profiling capped state (sleep).");
        EM_REG_PROFILE_ADV(&pVCpu->em.s.StatREMTotal,          "/PROF/CPU%d/EM/REMTotal",          "Profiling emR3RemExecute (excluding FFs).");
        EM_REG_PROFILE_ADV(&pVCpu->em.s.StatRAWTotal,          "/PROF/CPU%d/EM/RAWTotal",          "Profiling emR3RawExecute (excluding FFs).");
    ...
    if (rc == 0)
    {
        ...
        for (;;)
        {
            ...
            switch (pVCpu->em.s.enmState)
            {
                 ...
                 */
                case EMSTATE_RAW:
                    STAM_COUNTER_INC(&gStatSchedRAW);
                 ...
                 */
                case EMSTATE_HWACC:
                    STAM_COUNTER_INC(&gStatSchedHwAcc);
#ifndef IEM_VERIFICATION_MODE /* remove later */
                 ...
                 */
                case EMSTATE_REM:
                    STAM_COUNTER_INC(&gStatSchedREM);
#ifdef IEM_VERIFICATION_MODE
# if 1
