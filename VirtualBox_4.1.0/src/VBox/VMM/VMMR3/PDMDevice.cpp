...
int pdmR3DevInit(PVM pVM)
{
    ...
    for (pCur = CFGMR3GetFirstChild(pDevicesNode); pCur; pCur = CFGMR3GetNextChild(pCur))
    {
        ...
        dbg_log("00_device.log", "#### find device %s ---> %x\n", szName, pDev);
        ...
        for (pInstanceNode = CFGMR3GetFirstChild(pCur); pInstanceNode; pInstanceNode = CFGMR3GetNextChild(pInstanceNode))
        {
            ...
            dbg_log("00_device.log", "#### Get the instance number. %s ---> %d\n", szInstance, paDevs[i].iInstance);
     ...
     *
     */
    dbg_log("00_device.log", "#### PDM: cDevs=%d !\n", cDevs);
    for (i = 0; i < cDevs; i++)
    {
        /*
        ...
        dbg_log("00_device.log", "#### PDM: %d <> %d paDevs[i].pNode=%x !\n", i, cDevs, paDevs[i].pNode);
        bool fTrusted;
        ...
        dbg_log("00_device.log", "#### Constructing device %s ----> %d : %d\n", pDevIns->pReg->szName, pDevIns->iInstance, RT_FAILURE(rc));	
        if (RT_FAILURE(rc))
        {
    ...
    dbg_log("00_device.log", " ####  %s ---> %s\n", "pdmR3DevInit: returns VINF_SUCCESS", "OK");
...
static int pdmR3DevLoad(PVM pVM, PPDMDEVREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        dbg_log("00_device.log", "PDMR3LdrGetSymbolR3 %s ---> %s\n", pszFilename, pszName);
        if (RT_SUCCESS(rc))
        {
...
static DECLCALLBACK(int) pdmR3DevReg_Register(PPDMDEVREGCB pCallbacks, PCPDMDEVREG pReg)
{
    ...
    if (pDev)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            dbg_log("00_device.log", "PDM: Registered device '%s'\n", pReg->szName);
...
{
    VM_ASSERT_EMT(pVM);
    dbg_log("00_device.log", "PDMR3DeviceAttach: pszDevice=%p:{%s} iInstance=%d iLun=%d fFlags=%#x ppBase=%p\n",
...
{
    VM_ASSERT_EMT(pVM);
    dbg_log("00_device.log", "PDMR3DriverAttach: pszDevice=%p:{%s} iInstance=%d iLun=%d fFlags=%#x ppBase=%p\n",
             pszDevice, pszDevice, iInstance, iLun, fFlags, ppBase);
