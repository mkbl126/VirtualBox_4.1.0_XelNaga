...
static void cfgmR3DumpPathTXT(PCFGMNODE pNode, FILE* fp);
static void cfgmR3DumpTXT(PCFGMNODE pRoot, unsigned iLevel, FILE* fp);
...
/**
 * Dumps the configuration (sub)tree to the txt file.
 *
 * @param   pRoot   The root node of the dump.
 */
VMMR3DECL(void) CFGMR3DumpTXT(PCFGMNODE pRoot, const char* pszFilename)
{
    FILE* fp = fopen(pszFilename, "a");
    fprintf(fp, "************************* CFGM dump *************************\n");
    cfgmR3DumpTXT(pRoot, 0, fp);
    fprintf(fp, "********************* End of CFGM dump **********************\n");
    fclose(fp);
}
...
{
    if (pNode->pParent)
        cfgmR3DumpPathTXT(pNode->pParent, fp);
    fprintf(fp, "%s/", pNode->szName);
}
...
static void cfgmR3DumpTXT(PCFGMNODE pRoot, unsigned iLevel, FILE* fp)
{
    ...
    for (pLeaf = CFGMR3GetFirstValue(pRoot); pLeaf; pLeaf = CFGMR3GetNextValue(pLeaf))
    {
        ...
        {
            case CFGMVALUETYPE_INTEGER:
                //fprintf(fp, "  %-*s <integer> = %#018llx (%lld)\n", (int)cchMax, pLeaf->szName, pLeaf->Value.Integer.u64, pLeaf->Value.Integer.u64);
                fprintf(fp, "(%d)<integer> %s : %lld \n", (int)cchMax, pLeaf->szName, pLeaf->Value.Integer.u64);
                break;

            case CFGMVALUETYPE_STRING:
                //fprintf(fp, "  %-*s <string>  = \"%s\" (cb=%zu)\n", (int)cchMax, pLeaf->szName, pLeaf->Value.String.psz, pLeaf->Value.String.cb);
                fprintf(fp, "(%d)<string>  %s : %s (%d)\n", (int)cchMax, pLeaf->szName, pLeaf->Value.String.psz, pLeaf->Value.String.cb);
                break;

            case CFGMVALUETYPE_BYTES:
                //fprintf(fp, "  %-*s <bytes>   = \"%.*Rhxs\" (cb=%zu)\n", (int)cchMax, pLeaf->szName, pLeaf->Value.Bytes.cb, pLeaf->Value.Bytes.pau8, pLeaf->Value.Bytes.cb);
                fprintf(fp, "(%d)<bytes>   %s : %x (%d)\n", (int)cchMax, pLeaf->szName, (unsigned int)pLeaf->Value.Bytes.pau8, pLeaf->Value.Bytes.cb);
                break;
    ...
    for (PCFGMNODE pChild = CFGMR3GetFirstChild(pRoot); pChild; pChild = CFGMR3GetNextChild(pChild))
    {
        ...
        Assert(pChild->pFirstChild != pChild);
        Assert(pChild->pParent != pChild);
        cfgmR3DumpTXT(pChild, iLevel + 1, fp);
    }
}
