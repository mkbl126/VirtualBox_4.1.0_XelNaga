...
#include "00_syscall.h"
#include "string.h"

//static RTGCPTR32 XPPspCidTable = 0x0;

VMMDECL(bool) TRPMSimpleReadData(PVMCPU pVCpu, RTGCPTR ptr, void *pData, size_t uSize, bool bPrint, const char* szTitle)
{
    if ( ptr==0 || pData==NULL || uSize>1024 )
        return false;
    char buf[1025] = {0};
    int rc = PGMPhysSimpleReadGCPtr(pVCpu, pData, ptr, uSize);
    uint32_t align = 32;
    uint32_t center = align >> 1;
    //if ( bPrint==true )
    //    LogZ(("\n*********************** [%s : %d] ***********************\n", szTitle, RT_SUCCESS(rc)));
    if ( RT_SUCCESS(rc) && bPrint==true && szTitle!=NULL ) 
    {
        uint32_t idx = 0;
        for (uint32_t i=0, base = ptr; i<uSize; i+=align, base+=align)
        {
            LogZ(("%6s  0x%08X :", szTitle, base));
            for (uint32_t j=0; j<align; j++)
            {
                ...
                if ( j==center )
                    ...
                    LogZ((" "));
                if ( idx<uSize )
                    LogZ(("%02X", ((char*)pData)[idx] & 0x000000FF)); 
                else
                    LogZ(("  ")); 
                if ( ((char*)pData)[idx]<0x21 || ((char*)pData)[idx]>0x7E )
                    buf[j] = '.';
                else
                    buf[j] = ((char*)pData)[idx] ;
            }
...
VMMDECL(RTGCPTR32) XPLookUpObjectInTable(PVMCPU pVCpu, RTGCPTR32 pHandleTable, uint32_t hHandle)
{
    const int SIZE = 64;
    char buf[SIZE] = { 0 };
    RTGCPTR32 pEntry = 0, pObject = 0;
    RTGCPTR32 pLowLevelTable = 0, pMidLevelTable = 0, pHighLevelTable = 0;

    // decode HANDLE index
    uint32_t HighLevelTableIndex = (hHandle & 0x3e00000u) >> 21u;
    uint32_t MidLevelTableIndex = (hHandle & 0x1ff800u) >> 11u;
    uint32_t LowLevelTableIndex = (hHandle & 0x7fcu) >> 2u;

    int rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pHandleTable, (void *)buf, SIZE, false, "HandleTable");
    RTGCPTR32 TableCode = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
    uint32_t code = TableCode & 0x3;

    //LogZ(("++ HandleTable = %08X, TableCode = 0x%08X; Code = %u\n", pHandleTable, TableCode, code));
    switch( code )
    {
        case 0x00:
        {
            pLowLevelTable = TableCode & 0xfffffffc;
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pLowLevelTable+8*LowLevelTableIndex, SIZE);
            pEntry = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            break;
        }
        case 0x01:
        {
            pMidLevelTable = TableCode & 0xfffffffc;
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pMidLevelTable+sizeof(RTGCPTR32)*MidLevelTableIndex, SIZE);
            pLowLevelTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);            
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pLowLevelTable+8*LowLevelTableIndex, SIZE);
            pEntry = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            break;
        }
        case 0x02:
        {
            pHighLevelTable = TableCode & 0xfffffffc;
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pHighLevelTable+sizeof(RTGCPTR32)*HighLevelTableIndex, SIZE);
            pMidLevelTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pMidLevelTable+sizeof(RTGCPTR32)*MidLevelTableIndex, SIZE);
            pLowLevelTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);            
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pLowLevelTable+8*LowLevelTableIndex, SIZE);
            pEntry = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
    ...
    pObject = pEntry & 0xFFFFFFF8;
 /*
    LogZ(("++ Lid = 0x%08X, Mid = 0x%08X, Hid = 0x%08X, Ltab = 0x%08X, Mtab = 0x%08X, Htab = 0x%08X\n", 
           LowLevelTableIndex, MidLevelTableIndex, HighLevelTableIndex, pLowLevelTable, pMidLevelTable, pHighLevelTable));
    LogZ(("++ Entry = 0x%08X, Object = 0x%08X\n", pEntry, pObject));
*/  
    return pObject;
}

VMMDECL(RTGCPTR32) XPGetObjetcByHandle(PVMCPU pVCpu, RTGCPTR32 pEPROCESS, uint32_t hHandle)
{
    // return the base address of _OBJECT_HEADER, +0x18 for _OBJECT_BODY
    const int SIZE = 64;
    char buf[SIZE] = { 0 };
    RTGCPTR32 pEntry = 0, pObject = 0;
    
    if ( hHandle==0xFFFFFFF )
        return pEPROCESS;
        
    if ( hHandle==0xFFFFFFFE )
        return pObject;

    // find HandleTable in EPROCESS
    //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pEPROCESS, (void *)buf, SIZE, true, "EPROCESS");
    int rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pEPROCESS+0xC4, SIZE);
    //int rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pEPROCESS+0xC4, (void *)buf, SIZE, true, "DATA");

    RTGCPTR32 pHandleTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
    rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pHandleTable, SIZE);
    //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pHandleTable, (void *)buf, SIZE, true, "HandleTable");
    RTGCPTR32 TableCode = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
    RTGCPTR32 EP = (RTGCPTR32)*((RTGCPTR32 *)&buf[4]);
    uint32_t pid = (RTGCPTR32)*((RTGCPTR32 *)&buf[8]);
    LogZ(("++ HandleTable, EPROCESS = 0x%08X ( Quote by 0x%08X, %d )\n", pEPROCESS, EP, pid));

    pObject = XPLookUpObjectInTable(pVCpu, pHandleTable, hHandle);

    if ( pObject!=0x0 )
    {
        rc = TRPMSimpleReadData(pVCpu, pObject, (void *)buf, SIZE, false, "OBJECT");
        uint32_t pc = (uint32_t)*((uint32_t *)&buf[0]);     
        uint32_t hc = (uint32_t)*((uint32_t *)&buf[4]);     
        LogZ(("++ PointerCounter = %d, HandleCount = %d\n", pc, hc));   
    }
    
    return pObject;
}

VMMDECL(RTGCPTR32) XPGetEPROCESS(PVMCPU pVCpu, RTGCPTR32 fs, uint32_t pid)
{
    RTGCPTR32 pEPROCESS = 0x0;
    int rc = 0;
    const int SIZE = 64;
    char buf[SIZE] = { 0 };
    RTGCPTR32 *pXPPspCidTable = (RTGCPTR32 *)&XPPspCidTable[0];
    //LogZ(("*pXPPspCidTable %08X\n", *pXPPspCidTable));
    if ( fs & 0x80000000 )
    {
        // fs located in kernel space
        // KPCR -> KPCBR -> KTHREAD (ETHREAD)
        rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, fs+0x124, SIZE);
        RTGCPTR32 pETHREAD = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
        // ETHREAD -> KPROCESS
        rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pETHREAD+0x44, SIZE);
        pEPROCESS = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            
        if ( !(*pXPPspCidTable) )
        {
            // KPCR ->  KdVersionBlock -> [PspCidTable] 
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, fs+0x34, SIZE);
            //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)fs+0x34, (void *)buf, SIZE, false, "KdVersionBlock");
            RTGCPTR32 pKdVersionBlock = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pKdVersionBlock+0x80, SIZE);
            //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pKdVersionBlock+0x80, (void *)buf, SIZE, true, "PspCidTable");              
            RTGCPTR32 pPspCidTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
            //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pPspCidTable, (void *)buf, SIZE, true, "[PspCidTable]");
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pPspCidTable, SIZE);
            *pXPPspCidTable = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
        }
    }
    else
    {
        if ( (*pXPPspCidTable) )
        {
            pEPROCESS = XPLookUpObjectInTable(pVCpu, *pXPPspCidTable, pid);
            //rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)pEPROCESS, (void *)buf, SIZE, true, "EPROCESS");
        }
    }
    return pEPROCESS;
...
VMMDECL(int) XPGetObjectName(PVMCPU pVCpu, RTGCPTR32 pObject, char* szPath)
{
    const int SIZE = 512;
    char buf[SIZE] = { 0 };
    // Test if the object is file obejct, if not, just return.
    int rc = TRPMSimpleReadData(pVCpu, pObject+0x18, (void *)buf, SIZE, false, "OBJECT");
    uint16_t type = (uint16_t)buf[0x0];
    if ( type!=0x05 )
        return 0; 

    // FILE_OBJECT -> FILE_OBJECT.PDEVICE_OBJECT -> Device Object
    RTGCPTR32 PDeviceObjectBody = (RTGCPTR32)*((RTGCPTR32 *)&buf[4]);
    rc = TRPMSimpleReadData(pVCpu, PDeviceObjectBody, (void *)buf, SIZE, false, "DevObj");

    // Device Object -> OBJECT_HEADE -> NAME_OBJ_OFFSET
    char Drive[32] = {0}; 
    uint16_t len1 = 0, len2 = 0;    
    rc = TRPMSimpleReadData(pVCpu, PDeviceObjectBody-0x18, (void *)buf, SIZE, false, "DevObjHeader");
    uint32_t NameInfoOffset = (uint32_t)buf[0xC];
    if ( NameInfoOffset!=0x0 )
    {
        // NAME_OBJ -> UNICODE_STRING Name, Unicode String here, starts from buf[0x04] 
        rc = TRPMSimpleReadData(pVCpu, (RTGCPTR32)(PDeviceObjectBody-0x18-NameInfoOffset), (void *)buf, SIZE, false, "NAME_OBJ");
        len1 = (uint16_t)*((uint16_t *)&buf[0x4]);                                 
        RTGCPTR32 NameBuf = (RTGCPTR32)*((RTGCPTR32 *)&buf[0x8]);
        rc = TRPMSimpleReadData(pVCpu, NameBuf, (void *)Drive, 32, false, "NAME_BUF");
        memcpy(szPath, Drive, len1);
        //LogZ2(("++ ObjName : %S\n", Path));                    
    }
    // Get file path from FileName field in FILE_OBJECT, FILE_OBJECT -> FILE_OBJECT.FileName
    rc = TRPMSimpleReadData(pVCpu, pObject+0x18+0x30, (void *)buf, SIZE, false, "FNAME");
    // Unicode String here. UNICODE_STRING FileName                               
    len2 = (uint16_t)*((uint16_t *)&buf[0x0]);
    RTGCPTR32 FilePath = (RTGCPTR32)*((RTGCPTR32 *)&buf[4]);
    rc = TRPMSimpleReadData(pVCpu, FilePath, (void *)buf, SIZE, false, "NBUF");
    //LogZ(("Len 1, 2 %d, %d\n", len1, len2));
    memcpy(szPath+len1*sizeof(char), buf, len2);

    return 1;
}

/**
 * Intercepter trap before forward trap or interrupt to the guest's handler
 *
 *
 * @returns VBox status code.
...
VMMDECL(int) TRPMInterceptTrap(PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, uint32_t iGate, uint32_t opsize,
                             TRPMERRORCODE enmError, TRPMEVENT enmType, int32_t iOrgTrap)
{
    ...
    switch(iGate)
    {
        case 0x2E :
        {
            //LogZ(("TRPMInterceptTrap found iGate=%X, eip=%04X:%08X, NO.=0x%X, Args=%08X, OpSize=%d, fs=%04X\n", 
            //        iGate, pRegFrame->cs, pRegFrame->eip, pRegFrame->eax, pRegFrame->edx, opsize, pRegFrame->fs));
            const int SIZE = 256;
            char buf[SIZE] = { 0 };
            char name[SIZE] = { 0 };
            RTGCPTR32 pEPROCESS = 0x0000000;

            // dump the nearby code of the int instr.
            //TRPMSimpleReadData(pVCpu, (RTGCPTR32)(pRegFrame->eip-SIZE/2), (void *)OpCodes, SIZE, true, "CODE");
                        
            VBOXGDTR  Gdtr;
            RTGCPTR32 GCPtrDesc;
            CPUMGetGuestGDTR(pVCpu, &Gdtr);
            RTSEL fs = CPUMGetGuestFS(pVCpu);
            X86DESC Desc, *pDesc;
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)&Desc, (RTGCPTR32)(Gdtr.pGdt + (fs & X86_SEL_MASK)), sizeof(Desc));
            pDesc = &Desc;
            uint32_t u32Base = pDesc->Gen.u16BaseLow | ((uint32_t)pDesc->Gen.u8BaseHigh1 << 16) | ((uint32_t)pDesc->Gen.u8BaseHigh2 << 24);
            uint32_t cbLimit = pDesc->Gen.u16LimitLow | (pDesc->Gen.u4LimitHigh << 16);
            //LogZ(("\nfs=%04X, base=%08X, limit=%08X\n", fs, u32Base, cbLimit));

            // get process and thread related info of the caller thread.
            // fs:[0] -> TEB, fs:[30] -> PEB
            TRPMSimpleReadData(pVCpu, (RTGCPTR32)u32Base, (void *)buf, SIZE, false, "DATA");

            // the fs:[0] located in user space.       
            uint32_t tid = *((uint32_t*)&buf[0x24]);
            uint32_t pid = *((uint32_t*)&buf[0x20]);
            
            pEPROCESS = XPGetEPROCESS(pVCpu, u32Base, pid);
            // EPROCESS -> ImageName
            rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)name, pEPROCESS+0x174, 16); 

            if ( (u32Base & 0x80000000) )
            {
                // the fs:[0] located in kernel space.
                // KPCR -> KPCBR -> KTHREAD (ETHREAD)
                rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, u32Base+0x124, SIZE);
                RTGCPTR32 pETHREAD = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
                // ETHREAD -> RealClientID
                rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)buf, pETHREAD+0x1EC, SIZE);
                pid = *((uint32_t*)&buf[0x0]);
                tid = *((uint32_t*)&buf[0x4]);             
            }

            if ( pRegFrame->eax<COUNT_SYSCALL_WINXP &&  
                 ( strcmp(name, "agentbase.exe-0")==0 || 
                   strcmp(name, "write.exe")==0 || 
                   strcmp(name, "Untitled1.exe")==0 ||
                   strcmp(name, "AgentBase.exe")==0 || 
                   strcmp(name, "422c.exe-0")==0 || 
                   strcmp(name, "taskkill.exe")==0 ) )
            {
                LogZ(("\n==> %16s\t\tProcessID: %8u, Thread ID: %8u, CALLED 0x%4X = %s, ARGS @ 0x%08X\n",  
                     name, pid, tid, pRegFrame->eax, syscall_windows_xp[pRegFrame->eax], pRegFrame->edx));
                switch ( pRegFrame->eax )
                {
                    /*case 0x30:              // NtCreateProcessEx
                    {
                        LogZ(("\n==> ProcessID: %8u, Thread ID: %8u, CALLED 0x%4X = %s, ARGS @ 0x%08X\n",  
                               pid, tid, pRegFrame->eax, syscall_windows_xp[pRegFrame->eax], pRegFrame->edx));   
                        TRPMSimpleReadData(pVCpu, (RTGCPTR32)pRegFrame->edx, (void *)buf, SIZE, true, "DATA");
                        uint32_t uCreateFlags = *((uint32_t*)&buf[16]);

                        //if ( !(uCreateFlags & CREATE_SUSPENDED) )
                        //    LogZ(("!!!\n"));
                        break;
                    }*/
                    case 0x35:              // NtCreateThread
                    {
                        LogZ(("\nfs=%04X, base=%08X, limit=%08X\n", fs, u32Base, cbLimit));
                        TRPMSimpleReadData(pVCpu, (RTGCPTR32)pRegFrame->edx, (void *)buf, SIZE, false, "DATA");
                        uint32_t hProcHanle, bSuspend;
                        hProcHanle = *((uint32_t*)&buf[12]);
                        bSuspend = *((uint32_t*)&buf[28]);

                        //LogZ(("\n==> %s\t\tProcessID: %8u, Thread ID: %8u, CALLED 0x%4X = %s, ARGS @ 0x%08X\n",  
                        //        name, pid, tid, pRegFrame->eax, syscall_windows_xp[pRegFrame->eax], pRegFrame->edx));
                        LogZ(("++ NtCreateThread ARGS @ 0x%08X, HANDLE = %08X\n", pRegFrame->edx, hProcHanle ));   
                        
                        // Decode ProcessHandle
                        if ( pEPROCESS!=0x0 )
                        {
                            RTGCPTR32 pObject = XPGetObjetcByHandle(pVCpu, pEPROCESS, hProcHanle);
                            if ( pObject!=0x0 )
                            {
                                RTGCPTR32 pNewEPROCESS = pObject + 0x18;
                                //rc =  TRPMSimpleReadData(pVCpu, pNewEPROCESS, (void *)buf, SIZE, true, "EPROCESS");
                                rc =  TRPMSimpleReadData(pVCpu, pNewEPROCESS+0x84, (void *)buf, SIZE, false, "DATA");
                                uint32_t apid = *((uint32_t*)&buf[0x0]);
                                uint32_t atid = *((uint32_t*)&buf[0x4]);
                                char aName[SIZE] = {0};
                                rc = PGMPhysSimpleReadGCPtr(pVCpu, (void *)aName, pNewEPROCESS+0x174, 16); 
                                LogZ(("++ Target PID: %8u, TID: %8u\n", apid, atid)); 

                                // enumrate all threads
                                rc =  TRPMSimpleReadData(pVCpu, pNewEPROCESS+0x190, (void *)buf, SIZE, false, "THREAD_LIST");
                                RTGCPTR32 flink1 = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
                                RTGCPTR32 blink1 = (RTGCPTR32)*((RTGCPTR32 *)&buf[4]);
                                LogZ(("++ FLink : %08X, BLink : %08X\n", flink1, blink1)); 
                                           
                                rc =  TRPMSimpleReadData(pVCpu, flink1, (void *)buf, SIZE, false, "FLNIK");
                                rc =  TRPMSimpleReadData(pVCpu, blink1, (void *)buf, SIZE, false, "BLNIK");
                                
                                RTGCPTR32 flink2 = (RTGCPTR32)*((RTGCPTR32 *)&buf[0]);
                                RTGCPTR32 blink2 = (RTGCPTR32)*((RTGCPTR32 *)&buf[4]);
                                
                                if ( flink1!=flink2 )
                                    LogZ(("-- %s\tPID: %8u, TID: %8u, INJECTED --> %d : %s\n", name, pid, tid, apid, aName));
                                else 
                                    LogZ(("-- %s\tPID: %8u, TID: %8u, CREATED  --> %d : %s\n", name, pid, tid, apid, aName));

                                //rc =  TRPMSimpleReadData(pVCpu, flink2, (void *)buf, SIZE, false, "FLNIK");
                                //rc =  TRPMSimpleReadData(pVCpu, blink2, (void *)buf, SIZE, false, "BLNIK");
                            }
                        }
                    ...
                    case 0x112 :              // NtWriteFile
                    {
                        //LogZ(("\n==> %16s\t\tProcessID: %8u, Thread ID: %8u, CALLED 0x%4X = %s, ARGS @ 0x%08X\n",  
                        // name, pid, tid, pRegFrame->eax, syscall_windows_xp[pRegFrame->eax], pRegFrame->edx));
                        uint32_t hFileHandle = 0, Length = 0, Buffer = 0;
                        rc = TRPMSimpleReadData(pVCpu, pRegFrame->edx, (void *)buf, SIZE, false, "ARGS");
                        hFileHandle = (uint32_t)*((uint32_t *)&buf[0]);
                        Buffer = (uint32_t)*((uint32_t *)&buf[20]);
                        Length = (uint32_t)*((uint32_t *)&buf[24]);

                        // Parse File Object
                        RTGCPTR32 pObject = 0;
                        if ( pEPROCESS!=0x0 )
                        {
                            pObject = XPGetObjetcByHandle(pVCpu, pEPROCESS, hFileHandle);
                            if ( pObject!=0x0 )
                            {
                                //LogZ(("-- %s\tPID: %8u, TID: %8u, WriteFile --> %S%S, length : %u\n", name, pid, tid, Path, buf, Length));
                                char Path[SIZE] = {0};
                                XPGetObjectName(pVCpu, pObject, Path);
                                LogZ(("-- %s\tPID: %8u, TID: %8u, WriteFile --> %S, length : %u\n", name, pid, tid, Path, Length));
                                const uint32_t BUF_SIZE = 1024;
                                /*if ( Length<=BUF_SIZE ) 
                                {
                                    char buffer[BUF_SIZE] = { 0 };
                                    rc = TRPMSimpleReadData(pVCpu, Buffer, (void *)buffer, Length, true, "DATA");    
                                }*/
                            }
                        }
                    }
            ...
            {
                //LogZ(("\n==> ProcessID: %8u, Thread ID: %8u, CALLED 0x%4X = For shadow SSDT, ARGS @ 0x%08X\n",  
                //     pid, tid, pRegFrame->eax, pRegFrame->edx));
            }

            break;
        }
        default : ;
            //LogZ(("TRPMInterceptTrap @@@ iGate=%X\n", iGate));
     }
    return VINF_SUCCESS;
}
...
VMMDECL(int) TRPMForwardTrap(PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, uint32_t iGate, uint32_t opsize,
                             TRPMERRORCODE enmError, TRPMEVENT enmType, int32_t iOrgTrap)
{
    Log(("TRPMForwardTrap ### iGate=%X\n", iGate));
    TRPMInterceptTrap(pVCpu, pRegFrame, iGate, opsize, enmError, enmType, iOrgTrap);
...
# if defined(VBOX_STRICT) || defined(LOG_ENABLED)
    if (pRegFrame->eflags.Bits.u1VM)
        Log(("TRPMForwardTrap-VM: eip=%04X:%04X iGate=%X\n", pRegFrame->cs, pRegFrame->eip, iGate));
    else
        Log(("TRPMForwardTrap: eip=%04X:%08X iGate=%X\n", pRegFrame->cs, pRegFrame->eip, iGate));
